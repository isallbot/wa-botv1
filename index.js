...
const { help } = require('./is4ll/help')
const { bahasa } = require('./is4ll/bahasa')
const { donasi } = require('./is4ll/donasi')
...
async function starts() {
	...
	fs.existsSync('./is4ll.json') && client.loadAuthInfo('./is4ll.json')
	...
	await client.connect({timeoutMs: 30*1000})
        fs.writeFileSync('./is4ll.json', JSON.stringify(client.base64EncodedAuthInfo(), null, '\t'))
	...
	client.on('chat-update', async (mek) => {
		try {
			...
			const Isall = (type === 'chat') ? body : (type === 'image' || type === 'video') ? caption : ''
     ...
     mek.reply('too')
		...
		case 'daftar':
					...
					if (isBanned) return reply(mess.only.benned)
					if (args.length < 1) return reply(`Parameter Salah\nCommand : ${prefix}daftar nama/umur\nContoh : ${prefix}daftar ISALL/17/Banyuwangi`)
					reg = `${body.slice(8)}`
					jeneng = reg.split("/")[0];
					umure = reg.split("/")[1];
					asal = reg.split("/")[2];
            ...
            break
                 ...
                 case 'kalkulator':
				   ...
				   if (!isUser) return reply(mess.only.userB)
				    ...
				    break
				case 'owner':
                 client.sendMessage(from, {displayname: "ISALL GANS", vcard: vcard}, MessageType.contact, { quoted: mek})
         ...
         case 'lirik':
                ...
                if (!isUser) return reply(mess.only.userB)
                    ...
                    client.sendMessage('6289630267618@s.whatsapp.net', options, text, {quoted: mek})
...
//akhir kreator
		    ...
		    case 'darkjokes':
				...
				if (!isUser) return reply(mess.only.userB)
				reply(mess.wait)
				 data = fs.readFileSync('./is4ll/drak.js');
